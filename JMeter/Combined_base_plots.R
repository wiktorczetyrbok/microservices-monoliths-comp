...
for (mainFolder in mainFolders) {
  ...
  plot_range <- max(combinedData$Throughput) - min(combinedData$Throughput)
  
  user_range <- range(combinedData$Users)
  extended_range <- c(0, user_range[2])
  
  p <- ggplot(combinedData, aes(x = Users, y = Throughput, colour = SubFolder, group = SubFolder)) +
    geom_point(size = 2) +
    geom_line(linewidth = 1.5) +
    geom_text(data = combinedData[combinedData$FirstMax, ], aes(label = paste(Throughput), y = Throughput + (0.02 * plot_range)), hjust = -0.12, color = "black", size = 6) +
    geom_point(data = combinedData[combinedData$FirstMax, ],  aes(x = Users, y = Throughput), colour = "black", size = 3, shape = 17) +
    labs(title = basename(mainFolder), x = "Liczba użytkowników", y = "Przepustowość", color = "Implementacja") +
    theme_minimal() +
    theme(
  ...
  svg_filename <- paste0("C:/Users/pawel/OneDrive/Pulpit/PB/Results/result_plots/grouped/", basename(mainFolder), "_plot.png")
