...
delta <- 4
total_time <- 5
baseDir <- "C:/Users/pawel/OneDrive/Pulpit/PB/Results/data_grouped_by_language/"
...
for (languageFolder in languageFolders) {
  ...
  for (architectureFolder in architectureFolders) {
    ...
    p <- ggplot(combinedData, aes(x = Users, y = Throughput, colour = Configuration, group = Configuration)) +
      ...
      labs(title = paste(basename(architectureFolder)), x = NULL, y = NULL, color = "Parametry maszyny") +
      scale_color_manual(labels = c("standard_2" = "2 vCPU 8GB RAM", "standard_4" = "4 vCPU 16GB RAM", "standard_8" = "8 vCPU 32GB RAM"), values = c("standard_2" = "#386cb0", "standard_4" = "#fdb462", "standard_8" = "#7fc97f")) +
      theme_Publication() +
  ...
  combinedPlot <- grid.arrange(
    ...
    top = textGrob(paste("Wyniki skalowania wertykalnego: ", basename(languageFolder)), gp = gpar(fontsize = 16, font = 2)),
  ...
  ggsave(filename = paste0("C:/Users/pawel/OneDrive/Pulpit/PB/Results/result_plots/vertical_scaling/", basename(languageFolder), "_combined_comparison_plot.png"), plot = finalPlot, width = 16, height = 8 * length(archPlots) + 2, dpi = 600, bg = "white")
}
