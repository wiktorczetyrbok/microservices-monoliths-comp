...
def load_horizontal_data(base_dir: Path) -> pd.DataFrame:
        ...
        for impl_dir in instance_dir.iterdir():
            ...
            for iteration_dir in impl_dir.iterdir():
                ...
                for csv in iteration_dir.glob("users_*.csv"):
                    ...
                    throughput = compute_throughput(bench)
    ...
    return (
        df.groupby(
            ["scaling_type", "language", "implementation", "resource_level", "users"],
            as_index=False,
        )
...
def get_grid_step(y_max: float) -> int:
    if y_max <= 500:
        return 50
    if y_max <= 1500:
        return 250
...
def plot_language_comparison(language: str, lang_df: pd.DataFrame, out_dir: Path):
    ...
    all_values = [
        value
        for values in [h_2x, v_2x, h_4x, v_4x]
        for value in values
        if pd.notna(value)
    ]
    ...
    fig.suptitle(
        f"Porównanie mechanizmów skalowania dla języka {prettify_language(language)}",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )

    axes[0].set_ylabel("Przepustowość [req/s]", fontweight="bold")
    ...
    out_path = out_dir / f"{language}_scaling_comparison.pdf"
